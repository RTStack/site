...
const ProductDetails = ({ product }) => {
  ...
  const optionFields = [
    ...
    { key: "gpu", label: "GPU" },
    { key: "ram", label: "RAM" },
    { key: "storage", label: "Storage Type" },
    { key: "storageSize", label: "Storage Size" },
    { key: "screen", label: "Screen" },
    { key: "accessories", label: "Accessories" },
  ];

  const cpus = product.options?.cpu || [];
  const gpus = product.options?.gpu || [];
  const memories = product.options?.ram || [];
  const storages = product.options?.storage || [];
  const storageSizes = product.options?.storageSize || [];
  const screens = product.options?.screen || [];
  const accessories = product.options?.accessories || [];

  const [storageSize, setStorageSize] = useState(
    product?.storageSize || "256gb"
  );
  const [selectedSet, setSelectedSet] = useState(
    sets.length > 0 ? sets[0].id : "basic"
  );
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("User memilih:", selectedClient);
    // bisa lempar ke API, set form, dll
  };
              ...
              <div className="flex flex-col lg:flex-row gap-7.5 xl:gap-17.5">
                ...
                <div className="max-w-[539px] w-full">
                  ...
                  <form onSubmit={(e) => e.preventDefault()}>
                    <div className="flex flex-col gap-4.5 border-y border-gray-3 mt-7.5 mb-6 py-4">
                      {/* <!-- details item --> */}
                            ...
                            >
                              <div className="relative">
                                ...
                                >
                                  ...
                                  >
                                    <CheckboxIcon className="fill-[#3C50E0]" />
                                  </span>
                                </div>
                              </div>
                              <span>{label}</span>
                      ...
                      <div className="flex items-center gap-4 border-b border-gray-3 pb-6">
                        <div className="min-w-[65px]">
                      ...
                      {selectedSet === "custom" &&
                        optionFields.map(({ key, label }) => {
                          const items = product.options?.[key];
                          if (!items || items.length === 0) return null;

                          const selectedValue = eval(key); // assuming you have `const [cpu, setCpu]` dsb
                          const setSelectedValue = eval(
                              ...
                              <div className="flex items-center gap-4">
                                {items.map((item) => (
                                  ...
                                  >
                                    ...
                                    />
                                    <div
                                      className={`flex items-center justify-center w-4 h-4 border rounded mr-2 ${
                      ...
                      {selectedSet === "custom" && storageSizes.length > 0 && (
                        <div className="flex items-center gap-4">
                          <div className="min-w-[65px]">
                            <h4 className="font-medium text-dark">Storage:</h4>
                          </div>
                          <div className="flex items-center gap-4">
                            {storageSizes.map((item) => (
                              <label
                                key={item.key}
                                htmlFor={item.key}
                                className="flex cursor-pointer select-none items-center"
                              >
                                <input
                                  ...
                                  id={item.key}
                                  value={item.key}
                                  checked={storageSize === item.key}
                                  onChange={() => setStorageSize(item.key)}
                                  className="sr-only"
                                />
                                <div
                                  className={`flex items-center justify-center w-4 h-4 border rounded mr-2 ${
                                    storageSize === item.key
                                      ? "bg-blue border-blue"
                                      : "border-gray-400"
                                  }`}
                                >
                      ...
                      {/* Kalau basic, tampilkan default spec storage */}
                      {selectedSet === "basic" && (
                        <div className="flex items-center gap-4">
                          <div className="min-w-[65px]">
                            <h4 className="font-medium text-dark">Storage:</h4>
                          </div>
                          <div>{product.storageSize || "Default Storage"}</div>
                        </div>
                      )}
